...
import {
	...
	getTotalShopPurchaseAmount,
	getTotalBundlePurchaseAmount,
	getTotalCreditsRefilled
...
export default function AccountingDashboard() {
	...
	const [totalBundlePurchase, setTotalBundlePurchase] = useState<number | null>(
		null
	)
	const [totalShopPurchase, setTotalShopPurchase] = useState<number | null>(
		null
	)
	const [totalCreditsRefilled, setTotalCreditsRefilled] = useState<
		number | null
	>(null)
	...
	const fetchDailyData = async (
		...
		if (
			creditsResult.error ||
			spendResult.error ||
			spendGroupResult.error ||
			bundlePurchaseResult.error ||
			shopPurchaseResult.error ||
			creditsRefilledResult.error
		) {
		...
		const dailyBundlePurchase = getDailyData(
			bundlePurchaseResult.data,
			dateRange
		)
		const dailyShopPurchase = getDailyData(shopPurchaseResult.data, dateRange)
		const dailyCreditsRefilled = getDailyData(
			creditsRefilledResult.data,
			dateRange
		)
		...
		setTotalSpend(
			spendResult.data
				? Object.values(spendResult.data).reduce((a, b) => a + b, 0)
				: 0
		)
		setTotalSpendGroup(
			spendGroupResult.data
				? Object.values(spendGroupResult.data).reduce((a, b) => a + b, 0)
				: 0
		)
		setTotalBundlePurchase(
			bundlePurchaseResult.data
				? Object.values(bundlePurchaseResult.data).reduce((a, b) => a + b, 0)
				: 0
		)
		setTotalShopPurchase(
			shopPurchaseResult.data
				? Object.values(shopPurchaseResult.data).reduce((a, b) => a + b, 0)
				: 0
		)
		setTotalCreditsRefilled(
			creditsRefilledResult.data
				? Object.values(creditsRefilledResult.data).reduce((a, b) => a + b, 0)
				: 0
		)
			...
			<dl className='mx-auto mt-12 grid grid-cols-1 gap-px bg-gray-900/5 sm:grid-cols-2 lg:grid-cols-4'>
				...
				{totalSpend !== null && (
					<div
						...
						className='flex flex-wrap items-baseline justify-between gap-x-4 gap-y-2 bg-gray-800 px-4 py-10 sm:px-6 xl:px-8'>
						<dt className='text-sm font-medium leading-6 text-gray-300'>
							Total Spend on Individual Activities
						</dt>
						<dd className='w-full flex-none text-3xl font-bold leading-10 tracking-tight text-gray-200'>
							${totalSpend}
						</dd>
				...
				{totalSpendGroup !== null && (
					<div
						...
						className='flex flex-wrap items-baseline justify-between gap-x-4 gap-y-2 bg-gray-800 px-4 py-10 sm:px-6 xl:px-8'>
						<dt className='text-sm font-medium leading-6 text-gray-300'>
							Total Spend on Group Activities
						</dt>
						<dd className='w-full flex-none text-3xl font-bold leading-10 tracking-tight text-gray-200'>
							${totalSpendGroup}
						</dd>
				...
				<ResponsiveContainer width='100%' height={400}>
					<LineChart data={chartData}>
						...
						<Line
							...
							stroke='#8884d8'
							name='Total Active Credits'
						/>
						<Line
							type='monotone'
							dataKey='totalSpend'
							stroke='#82ca9d'
							name='Total Spend on individual activities'
						/>
						<Line
							type='monotone'
							dataKey='totalSpendGroup'
							stroke='#ffc658'
							name='Total Spend on Group activities'
						/>
						<Line
							type='monotone'
							dataKey='totalBundlePurchase'
							stroke='#ff7300'
							name='Total spend on Bundle Purchase'
						/>
						<Line
							type='monotone'
							dataKey='totalShopPurchase'
							stroke='#387908'
							name='Total spend on Shop Purchase'
						/>
						<Line
							type='monotone'
							dataKey='totalCreditsRefilled'
							stroke='#ff0000'
							name='Total spend on Credits Refilled'
						/>
					</LineChart>
				</ResponsiveContainer>
			</div>
