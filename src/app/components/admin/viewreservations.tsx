...
export default function ViewReservationsComponent() {
    ...
    const handleBookedFilterChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const applyFilters = (data: Reservation[]) => {
        return data.filter((reservation) => {
            const activityMatch = filter.activity ? reservation.activity?.name.toLowerCase().includes(filter.activity.toLowerCase()) : true;
            const coachMatch = filter.coach ? reservation.coach?.name.toLowerCase().includes(filter.coach.toLowerCase()) : true;
            const userMatch = filter.user ?
                (reservation.user?.first_name.toLowerCase().includes(filter.user.toLowerCase()) ||
                    reservation.user?.last_name.toLowerCase().includes(filter.user.toLowerCase()))
                : true;
            const searchTermMatch = searchTerm ?
                (reservation.activity?.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                    reservation.coach?.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
            ...
            const bookedMatch = bookedFilter === 'all' ? true :
                (bookedFilter === 'booked' ? reservation.booked === true : reservation.booked === false);
            ...
            const startTimeMatch = filter.startTime ? reservation.start_time >= filter.startTime : true;
            const endTimeAdjusted = filter.endTime ? addOneMinuteToTime(filter.endTime) : null;
            const endTimeMatch = endTimeAdjusted ? reservation.end_time <= endTimeAdjusted : true;


            return activityMatch && coachMatch && userMatch && searchTermMatch && bookedMatch && dateMatch && startTimeMatch && endTimeMatch;
        });
    };
            ...
            <div className="mb-4">
                ...
                {showFilters && (
                    <div className="absolute left-0 mt-1 bg-white p-4 rounded shadow-lg z-10">
                        {/* Existing filters */}
                        <input type="text" name="activity" placeholder="Filter by Activity..." value={filter.activity} onChange={handleFilterChange} className="mb-2 block w-full" />
                        <input type="text" name="coach" placeholder="Filter by Coach..." value={filter.coach} onChange={handleFilterChange} className="mb-2 block w-full" />
                        <input type="text" name="user" placeholder="Filter by User..." value={filter.user} onChange={handleFilterChange} className="mb-2 block w-full" />
                        <select onChange={handleBookedFilterChange} value={bookedFilter} className="block w-full">
                            <option value="all">All</option>
                            <option value="booked">Booked</option>
                            <option value="notBooked">Not Booked</option>
                        </select>
                        {/* New Date Filter */}
                        <input type="date" name="date" placeholder="Filter by Date..." value={filter.date} onChange={handleFilterChange} className="mb-3 border block w-full" />
                        {/* Start Time Filter */}
                        <input type="time" name="startTime" placeholder="Start Time..." value={filter.startTime} onChange={handleFilterChange} className="mb-3 border block w-full" />
                        {/* End Time Filter */}
                        <input type="time" name="endTime" placeholder="End Time..." value={filter.endTime} onChange={handleFilterChange} className="mb-3 border block w-full" />

                        <div className="flex justify-between mt-4">
                            {/* Close Button */}
                            <button onClick={() => setShowFilters(false)} className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded transition duration-150 ease-in-out">
                                Close
                            </button>

                            {/* Reset Filters Button */}
                            <button onClick={() => {
                                setFilter({ activity: '', coach: '', user: '', date: '', startTime: '', endTime: '' });
                                setBookedFilter('all');
