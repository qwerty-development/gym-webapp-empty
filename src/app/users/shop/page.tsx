            ...
            <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12'>
                ...
                <div className="md:hidden sticky top-0 z-20 bg-gray-800 py-2 mb-4">
                    <div className="flex justify-center space-x-2">
                        <button
                            className={`px-4 py-2 rounded-full text-sm font-medium transition-colors duration-200 ${!isBundles ? 'bg-green-500 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}
                    ...
                    <ul>
                        <li className={`mb-5 text-white  p-2 px-6 ${!isBundles ? 'bg-green-500 ' : ''}`}>
                            <button
                                ...
                                onClick={() => setIsBundles(false)}
                            >
                                <img src="https://www.svgrepo.com/show/307607/food-and-drink-food-edible-healthy.svg" className="mr-2 h-8 w-8 filter invert" alt="Items Icon" />
                                Items
                            </button>
                        </li>
                        <li className={`mb-10 text-white p-2 px-6 ${isBundles ? 'bg-green-500' : ''}`}>
                            <button
                                className="flex items-center hover:text-green-400 w-full text-left"
                                onClick={() => setIsBundles(true)}
                            >
                                <img src="https://www.svgrepo.com/show/371744/bundle.svg" className="mr-2 h-8 w-8 filter invert" alt="Bundles Icon" />
                                Bundles
                            </button>
                        </li>
                    </ul>
                ...
                <div className='relative mt-4'>
                    <button onClick={toggleCart} className='fixed right-4 bottom-4 z-50 flex items-center bg-white text-green-500 p-2 rounded-full hover:bg-gray-200'>
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="feather feather-shopping-cart h-6 w-6">
                            <circle cx="9" cy="21" r="1"></circle>
                            <circle cx="20" cy="21" r="1"></circle>
                            <path d="M1 1h4l1.68 8.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"></path>
                        </svg>
                        {cart.length > 0 && (
                            <span className='ml-2 text-sm bg-green-500 text-white rounded-full px-2 py-1'>{cart.length}</span>
                        )}
                    </button>
                    {isBundles ? (
                        <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {items.map(item => (
                                <div key={item.id} className="bg-gray-800 rounded-lg shadow-lg p-6 flex flex-col items-center">
                                    <h2 className="text-2xl font-bold text-white mb-4">{item.name}</h2>
                                    <p className="text-lg text-green-400 mb-4">{item.price.toFixed(2)} credits</p>
                                    <button
                    ...
                    {isCartOpen && (
                        <div className='fixed top-0 right-0 w-full md:w-1/3 h-full bg-gray-900 p-6 overflow-auto z-40'>
                            ...
                            {cart.map(cartItem => (
                                <div key={cartItem.id} className='flex justify-between items-center mb-4'>
                                    ...
                                    <div className='flex items-center'>
                                        <span className='text-green-400 mr-4'>{(cartItem.price * cartItem.quantity).toFixed(2)} credits</span>
                                        <button
                            ...
                            <div className='border-t border-gray-700 pt-4'>
                                <h3 className='text-xl font-bold text-white'>Total: {getTotalPrice()} credits</h3>
                            </div>
                            <button onClick={handlePurchaseClick} className='mt-6 px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600'>
                                Buy
                            </button>
                            <button onClick={toggleCart} className='mt-6 px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600'>
                                Close
                            </button>
                        </div>
                    )}
